...
\lx to insert a new line. In this situation use the tilde $\tilde{}$ 
...
To force extra commands between specific lines, use
...
It is sometimes convenient to split a LaTeX source file into several
...
pictures and diagrams. Its use is very similar to the \verb+table+
environment discussed above.  Most commonly, plots etc.\ are in
PostScript (.ps)  or Encapsulated PostScript (.eps) form and may be included in a
document via the \verb+\includegraphics+ command.
Alternatives are (among others) the \verb+epsf+ or \verb+psfig+ packages which will not be
discussed here.\\

First, the graphics package must be loaded by including the
command
\begin{verbatim}
\usepackage[dvips]{graphics}
\end{verbatim}
...
The numerical argument of \verb+\scalebox+ can be changed to alter the size of the
picture.\\
...
section~\ref{start} and the sample file wpdoc.tex. 
...
The \verb+\label+ statement means that you may refer to the
...
the same way as for normal text). Note that spaces are ignored in math mode: to
...
For example, many publishers provide style files to be used by contributing
authors which set up articles in their preferred style which can be downloaded
from their website. Many common packages are already available on the Maths
Department machines. There are also in-house style files available for preparing
tutorial sheets, exam papers etc. These are accessed via the \verb+\usepackage+
command in the preamble, e.g.
\begin{center}
\verb+\usepackage{exam}+
\end{center}
will include the style file \textbf{exam.sty}. Many style files are accessible
from the UK\TeX archive \texttt{http://www.tex.ac.uk}.\\

Any personal style files should be stored in a subdirectory \texttt{texmf/tex} of
your home directory. This directory (and all subdirectories under it) are searched
each time the \verb+latex+ command is typed. Note that you can also use this
directory to store e.g.\ frequently accessed Postscript files (so that they can be
automatically detected by \lx).

\section{Slides and presentations}
There is more than one way to prepare slides in \lx. The most basic it to use the
\verb+\documentclass{slides}+ style, which is very similar to the \verb+article+
style except that the slide environment
\begin{center}
\verb+\begin{slide} ... \end{slide}+
\end{center}
is used to produces a series of separate slides.\\

An alternative is to use a more sophisticated package, e.g.\ the \verb+prosper+
package which is designed for data projectors, generating \textbf{.pdf} files to be viewed
using Adobe Acrobat. It is used by using the \verb+latex+ and \verb+dvips+
commands in the usual way (to obtain a file e.g.\ \textbf{talk.ps}) and then using
\begin{center}
\verb+ps2pdf talk.ps talk.pdf+
\end{center}
to obtain the viewable .pdf file \textbf{talk.pdf}. Helpful instructions and some
sample files can be found at \texttt{http://www.maths.man.ac.uk/$\sim$mheil/Prosper/}.
